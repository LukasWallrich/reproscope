...
- **2026-09-03, rebuild after the efficiency audit.** The audit's work list (docs/HANDOFF.md of that morning) is implemented: deterministic arbitration with cheap-vision batches, one strong call for contracts plus redacted methods with local leak repair, the leak scan limited to inferential and headline quantities, CONTRACT.json grouped per analysis, abstained match rows, one shared focal-claim binding, a targeted arm on the focal quantity only, Stage 2 scoped to the focal analysis, Stage 3 capped at 64 executed specifications, content-hash cache keys with per-step input and prompt checks, `--force-step`, and complete ledgering. Matching, targeted arm, reviews, multiverse and reports were rebuilt on the pilot's replica outputs for all five papers: USD 0.49 metered and USD 27 list-equivalent including retries, or USD 1.9–6.0 list-equivalent per paper for a single clean pass of Stages 1–3. Claude Opus returned "529 Overloaded" on 15 strong-call attempts during the rebuild. Five of the seven replicas marked failed re-execute cleanly once the checker runs the script in its isolation copy and from its own directory; the two DeepSeek scripts on Petersen import scipy and re-execute once the checker builds a per-replica environment with the base stack. Writeup: docs/evaluation/PILOT_EVALUATION.html.
- **2026-09-03, evening decisions.** Codex shadow prices set to OpenAI input list prices (Sol 4.0, Luna 0.2 USD per Mtok). Leak scan widened by analysis (every numeric claim of a result-carrying analysis). Reversed two-group contrasts graded on the flipped value with mirrored CI bounds; Axt's eight sign-artefact fails become seven band-A cells and one genuine inconsistency (the printed CI [.42, .93] for d = .45). Stage 3: unimplementable factors listed, each level tagged estimate/inference/reporting, reporting-only factors pinned, abstention when nothing moves the estimate or its significance, per-specification alpha. Replicas declare packages; the checker builds a per-replica environment. Stages 3 and reports rebuilt on all five papers: Ohtsubo 6 specs (rank 1), Hurst 16 (rank 1), Axt 13 (rank 1), Petersen 2 (identical estimate), Hertel 24 (rank 7 of 24; sphericity and error-term choices move F from 5.4 to 7.5). Rebuild spend for the whole day: USD 0.98 metered, USD 46 list-equivalent.
- **2026-09-05.** Replica agents and the re-execution checker share one Python environment outside the repository (`~/.cache/reproscope/replica-env`); the agent environment is scrubbed of the repository path before launch. Stage 0 ran on the new code on a Hertel copy: 22 calls, USD 0.06 metered, 2.29 list-equivalent; arbitration merged 82 of 97/89 entries deterministically with one cheap batch and no strong call; 101 claims, 27 contracts, 11 bindable; scan clean; focal F = 6.20 bound. GLM-5.3-flash answers a capped structured call (OpenRouter `reasoning.max_tokens`) with reasoning only after about 900 s, so the cap is opt-in and unused.
- **2026-09-07 to 09-08, first end-to-end run on the rebuilt code (Hertel, fresh id `Hertel_ClinPsychSci_2018_YabW_v2`).** Six defects surfaced, all fixed and pushed (c693575 to 70195d6): the leak scanner took degrees of freedom inside the printed statistic ("t(29)", "F(1, 24)") as forbidden numbers, which collided with the sample sizes the contract must state and blocked Stage 1; glm-5.3-flash answered one extraction chunk with an empty claim list on a call reported as successful, and arbitration went on with one extractor (78 claims instead of 109) until the stage learned to retry an empty extractor and refuse when it stays empty; Stage 0 marked itself done with a dirty scan; Stage 0's steps cached on the paper's external inputs only, so a rebuilt extraction did not rebuild arbitration; the focal binder matched the "1" in "F(1, 48)" to a cutoff mean of 1.0 from another analysis and ranked it above the focal F, so the targeted arm found no replica row and Stage 3 built its grid on the wrong quantity (the pilot's Hertel run is unaffected: it binds to the same F); and the Stage 3 executor spent its 2400 s deparsing afex and car source for the sphericity levels, left 3 of 63 rows, and the stage ranked them and paid a strong call to read them. The Claude subscription hit its session limit mid-run; claude -p reported it as an exit code 1 with an empty message, and the retry runner spent eight passes against it. Final Hertel v2: 109 claims, 22 contracts, scan clean; 8 replicas (opus 1, fable 1, luna 2, glm 2, deepseek 2), 251 of 337 match rows band A; six replicas reproduce the focal F = 6.20 in band A, both Luna replicas miss it, the targeted arm reaches it on its first attempt from deepseek_2's script; 63 specifications all converged, F from 5.40 to 7.46, median 5.60, reported value at rank 37 of 63 (extremeness 0.33), every specification significant. The executor's successful run took 40 minutes. Spend for the paper including every failed pass: USD 0.77 metered, 12.6 list-equivalent; DeepSeek replicas account for 0.29 of the metered part (10.5M input tokens).
